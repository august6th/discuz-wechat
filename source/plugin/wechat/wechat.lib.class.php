...
class WeChatServer
{
            ...
            case 'event':
                ...
                switch ($result['event']) {
                    ...
                    case 'click': // 自定义菜单事件
                        ...
                        $this->replyEventOfClick($result['key']);
                        break;
    ...
    public function replyEventOfClick($key)
    {
        global $_G;
        switch ($key) {
            case 'SIGN' :
                $openid = self::$_from_id;
                $result = C::t('#wechat#common_member_wechat')->fetch_by_openid($openid);
                if (empty($result)) {
                    echo $this->getXml4Txt("您还没有绑定论坛账号！\n\n<a href='$_G[siteurl]api.php?mod=bind&openid=$openid'>点此绑定</a>");
                } else {
                    $_G['setting']['get_pack_num'] = '3';
                    $num = isset($_G['setting']['get_pack_num']) ? dintval($_G['setting']['get_pack_num']) : '';
                    $uid = $result['uid'];
                    $increnum = rand(1, 6);
                    // 判断用户是否签到
                    $result = C::t('#wechat#common_member_wechat_sign')->fetch_by_uid($uid);
                    // $result 为空就进行第一次签到。
                    if (empty($result)) {
                        updatemembercount($uid, array('2' => $increnum), true, 'OAS', 666);
                        $data = array(
                            'uid' => $uid,
                            'status' => 1,
                        );
                        C::t('#wechat#common_member_wechat_sign')->insert($data);
                        if ($num) {
                            $un_get_num = $num - 1;
                            echo $this->getXml4Txt("红包领取成功！/:jump \n\n您还有 $un_get_num 个红包待领取！\n\n获得 $increnum 金钱！ <a href='$_G[siteurl]home.php?mod=spacecp&ac=credit&op=base&mobile=2'>查看详情</a>");
                        } else {
                            echo $this->getXml4Txt("红包领取成功！/:jump \n\n获得 $increnum 金钱！ <a href='$_G[siteurl]home.php?mod=spacecp&ac=credit&op=base&mobile=2'>查看详情</a>");
                        }
                    } else {
                        if ($num) {
                            if ($result['status'] < $num) {
                                ...
                                C::t('#wechat#common_member_wechat_sign')->update($uid, $updata);
                                $un_get_num = $num - $result['status'] - 1;
                                echo $this->getXml4Txt("红包领取成功！/:jump \n\n您还有 $un_get_num 个红包待领取！\n\n获得 $increnum 金钱！ <a href='$_G[siteurl]home.php?mod=spacecp&ac=credit&op=base&mobile=2'>查看详情</a>");
                            } else {
                                echo $this->getXml4Txt("您已经领取完所有红包了！明天再来吧！/::P");
                            }
                        } else {
                            echo $this->getXml4Txt("您今天已经领取过红包了！明天再来吧！/::P");
                        }
                    }
                }
                break;
